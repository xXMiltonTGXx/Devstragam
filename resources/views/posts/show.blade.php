        ...
        @auth
            @if ($post->user_id === auth()->user()->id)
                <form action="{{route('posts.destroy', $post)}}" method="POST">
                    @method('DELETE')
                    {{-- metodo spoofing te permite agregar otro tipo de peticiones, ya que en navegador solo acepta get y post --}}
                    {{-- metodos como put, patch o delete --}}
                    @csrf
         ...
         <form action="{{route('comentarios.store',['post'=>$post, 'user' =>$user])}}" method="POST">
            ...
            <div class="mb-5">
                ...
                <textarea
                    id="comentario"
                    name="comentario" 
                    placeholder="Agrege un comentario"
